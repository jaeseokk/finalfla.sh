    ...
    <div className={clsx([styles.Materials, { [styles.active]: active }])}>
      <ul>
        ...
        {animationUnits.map(({ id }, i) => {
          ...
          return (
            <li className={clsx({ [styles.selected]: selected })}>
              <div
                style={{
...
const NormalKnob = React.forwardRef<HTMLDivElement, NormalKnobProp>(
  ({ index, onExpand }, ref) => {
    const id = index < 0 ? null : animationUnits[index].id
    const patternId = id ? `icon-${id}` : undefined
    const fill = patternId ? `url(#${patternId})` : '#fff'

    return (
      <div className={styles.NormalKnob} ref={ref}>
        <svg
          version="1.1"
		...
		C81.83,67.47,83.09,65.11,83.16,61.95z"
          ...
          <g
            ...
            style={{
              transform: `rotate(${(index + 1) * 4.86}deg)`,
            }}
          >
            <g>
              <path
...
const Knob: React.FC<KnobProp> = ({ index, playing, onClick }) => {
  const normalKnobRef = useRef<HTMLDivElement>(null)
  const [expanded, setExpanded] = useState(false)
  const handleClickKnob = useCallback(() => {
    setExpanded(true)
  }, [])
  const knobBoundingClientRect = normalKnobRef.current?.getBoundingClientRect()
  const initialPosition = knobBoundingClientRect
    ? {
        x: knobBoundingClientRect.left,
        y: knobBoundingClientRect.top,
      }
    : { x: 0, y: 0 }
